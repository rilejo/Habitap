...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ¡ACTUALIZADO con tus credenciales!
    ...
    const renderComunicado = (doc) => {
        ...
        `;
        // Usamos prepend para que los nuevos aparezcan arriba
        comunicadosList.prepend(card);
    };
    ...
    db.collection('comunicados').orderBy('createdAt', 'desc').onSnapshot(snapshot => {
        ...
        if (snapshot.empty) {
            comunicadosList.innerHTML = '<p>No hay comunicados para mostrar. ¡Crea el primero!</p>';
        } else {
        ...
        comunicadosList.innerHTML = '<p>Error al cargar los comunicados. Revisa las reglas de seguridad de Firestore y la conexión.</p>';
    });
